document.addEventListener('DOMContentLoaded', function () {
	enableMouseMiddleClick();

	var urlParts = location.href.split('/');
	if (!needsToBeSkipped(urlParts)) {
		extractMainContent(urlParts);
		enableLinkHinting(urlParts);
	}
}, false);

function needsToBeSkipped(urlPartsArray) {
	return false;
}

function extractMainContent(urlPartsArray) {
	...
	if (urlPartsArray[2] == 'www.vedomosti.ru') {
		var content = document.getElementById('idmasha').innerHTML;
		body.innerHTML = content;
		body.innerHTML += custom_css;

	} else if (urlPartsArray[2] == 'www.securitylab.ru') {
		var content = $('div.main_content').html();
		body.innerHTML = content;
		body.innerHTML += custom_css;

	} else if (urlPartsArray[2] == 'habrahabr.ru' && urlPartsArray[3] == 'post') {
...
function enableLinkHinting(urlPartsArray) {
...
function enableMouseMiddleClick() {
